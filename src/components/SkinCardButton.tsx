...
const SkinCardButton: React.FC<Props> = ({
  product,
  setOpenModal,
  setProductId,
  isFavorite,
  ...
  const handleAddtoFavorite = () => {
    AddToFavorite({ userId: useId, productId: product.id });
    console.log("Add to favorites", product.id);
  };

  const handleRemoveFromFavorite = () => {
    // Logic to remove from favorite products
    Remove({ userId: useId, productId: product.id });
          ...
          <div className="flex justify-center items-center gap-2 mt-3 mx-auto border border-[rgb(134,3,21)] rounded-full py-1 px-5 w-max lg:text-[clamp(14px,1rem,1rem)]">
            ...
            {isFavorite ? (
              <button
                ...
                type="button"
                className="cursor-pointer flex justify-center items-center w-8 h-8 rounded-full bg-[rgb(134_3_21)] hover:bg-[#8603151a] text-white text-[clamp(14px,1rem,1rem)] transition-all duration-200 ease-in-out"
              >
                <FavoriteBorderIcon className="text-white hover:text-red-900  transition-all duration-300" />
              </button>
            ) : (
              <button
                type="button"
                onClick={handleAddtoFavorite}
                className="cursor-pointer flex justify-center items-center w-8 h-8 rounded-full bg-[#8603151a] text-[rgb(134,3,21)] text-[clamp(14px,1rem,1rem)] transition-all duration-200 ease-in-out"
              >
